...
export class RabbitAgentEnvironment extends AgentEnvironment {
    ...
    async registerHandler(handler: EnvironmentHandler): Promise<void> {
        ...
        await this.channel.basicConsume(queueName, {noAck: false, exclusive: true}, async (msg) => this.processNewInstruction(msg)).catch(error => this.logger.error(error))

        this.logger.info(`listening on worker identifier queue ${thisQueueName}`)
        await this.channel.basicConsume(thisQueueName, {noAck: false, exclusive: true}, (msg) => this.processDirectMessage(msg)).catch(error => this.logger.error(error))
    }
